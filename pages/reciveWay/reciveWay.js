...
Page({
    ...
    createorder: function(){
      ...
      if (!/^1\d{10}$/.test(that.data.telNumber)) {
        wx.showModal({
          title: '',
          showCancel: false,
          content: '请输入正确的手机号',
      ...
      if (!that.data.postalCode) {
        ...
        // } else {
            ...
            param.shiptype = 2;
            param.username = that.data.userName;
            param.usermobile = that.data.telNumber;
            param.useraddressarea = that.data.allAddressInfo.provinceName;
            param.useraddressareainfo = that.data.address;
            param.useraddresspostalcode = that.data.postalCode;
